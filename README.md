...
- `데이터분석_4_API(codex_refactored).ipynb`
- `exhibition_data_analysis.py`: GitHub/Codex에서 Python 코드를 쉽게 확인하기 위한 읽기용 스크립트
- 실행 환경: Google Colab
...
`gradio_app.py`는 Colab 전용 `ipywidgets` UI를 Windows 브라우저에서도 시험할 수 있도록 만든 별도 앱입니다.
...
모드에서 `파이썬 코드 자동 실행`을 선택하면 Gemini가 반환한 Python 코드 블록을 제한된 분석 환경에서 실행하고 표와 차트를 별도 결과 영역에 표시합니다. 파일·프로세스·네트워크 접근과 임의 모듈 import는 차단됩니다.
