...
3. **전처리 MVP**: 결측·특정값 테이블 일괄 처리, Column 삭제, 날짜 요소 분리, 결과 비교·다운로드 — 개선 완료
4. **시각화 MVP**: 6개 차트 계열, Pydantic 검증, row×column subplot, 텍스트/메뉴 요청, 통계 source·다운로드 — 1차 완료
...
- 대용량 데이터에서 전처리 테이블의 페이지·일괄 처리 한도를 어떻게 둘지
- 차트 라이브러리를 Matplotlib/Seaborn 중심으로 유지할지 Plotly 상호작용을 추가할지

기본 권장은 **Gemini 단일 공급자, 자동 실행 기본 비활성화, Matplotlib/Seaborn + 필요한 차트만 Plotly**다. 이 선택은 기존 기능을 보존하면서 Streamlit 상세 전처리·시각화를 단계적으로 확장하기 쉽다.
